...
If you prefer a graphical interface over the terminal, download the Claude Code desktop app for [Mac](https://claude.ai/api/desktop/darwin/universal/dmg/latest/redirect) or [Windows](https://claude.com/download). The desktop app provides the same capabilities in a dedicated window.

### Option 4: Web app (no install)

Try Claude Code instantly at [claude.ai/code](https://claude.ai/code). No installation required. The CLI and desktop app offer deeper integration with your local filesystem, but the web app is the fastest way to start.
